...
import sys
sys.path.insert(0, str(Path(__file__).resolve().parents[1] / "01-ops" / "life-os" / "scripts"))
...
class TestRepoHealth(unittest.TestCase):
    ...
    def test_run_command_not_found(self):
        ...
        result_code, stdout, stderr = repo_health.run_command(["nonexistent_command_12345"])
        self.assertEqual(result_code, 1)
    ...
    def test_check_git_status_clean_repo(self, mock_print, mock_run):
        ...
        with patch.object(repo_health, 'REPO_ROOT', self.temp_path):
            ...
            self.assertTrue(any("No uncommitted changes" in call for call in print_calls))
            self.assertTrue(any("Remote origin configured" in call for call in print_calls))
    ...
    def test_check_git_status_dirty_repo(self, mock_print, mock_run):
        ...
        with patch.object(repo_health, 'REPO_ROOT', self.temp_path):
            ...
            print_calls = [call[0][0] for call in mock_print.call_args_list]
            self.assertTrue(any("Uncommitted changes: 2 files" in call for call in print_calls))
    ...
    def test_check_file_integrity_all_present(self, mock_print):
        ...
        with patch.object(repo_health, 'REPO_ROOT', self.temp_path):
            ...
            self.assertGreaterEqual(checkmark_count, 7)  # At least 7 required files + commands
    ...
    def test_check_file_integrity_missing_files(self, mock_print):
        ...
        with patch.object(repo_health, 'REPO_ROOT', self.temp_path):
            ...
            missing_count = sum(1 for call in print_calls if "❌" in call and "missing" in call)
    ...
    def test_check_python_health_all_good(self, mock_print, mock_run):
        ...
        with patch.object(repo_health, 'REPO_ROOT', self.temp_path):
            ...
            print_calls = [call[0][0] for call in mock_print.call_args_list]
            self.assertTrue(any("Python files compile successfully" in call for call in print_calls))
            self.assertTrue(any("No ruff linting issues" in call for call in print_calls))
    ...
    def test_check_csv_health_success(self, mock_print, mock_run):
        ...
        with patch.object(repo_health, 'REPO_ROOT', self.temp_path):
            ...
            self.assertTrue(any("CSV validation passed" in call for call in print_calls))
    ...
    def test_check_security_clean(self, mock_print):
        ...
        with patch.object(repo_health, 'REPO_ROOT', self.temp_path):
            ...
            print_calls = [call[0][0] for call in mock_print.call_args_list]
            self.assertTrue(any("No obvious security issues found" in call for call in print_calls))
    ...
    def test_check_security_with_issues(self, mock_print):
        ...
        with patch.object(repo_health, 'REPO_ROOT', self.temp_path):
            ...
            self.assertTrue(any("Environment file found" in call for call in print_calls))
            self.assertTrue(any("Potential private key" in call for call in print_calls))
    ...
    @patch('builtins.print')
    def test_main_function(self, mock_print, mock_security, mock_csv, mock_test,
                          mock_python, mock_file, mock_git):
        ...
        print_calls = [call[0][0] for call in mock_print.call_args_list]
        self.assertTrue(any("Life-OS Repository Health Check" in call for call in print_calls))
